...
export class DissolveTextureEffect extends TextureEffect {
  public static create(engineState: IEngineState, speed: number): DissolveTextureEffect {
    return new DissolveTextureEffect(engineState, speed);
  }
  ...
  public apply(texture: number[][]): number[][] {
    ...
    texture.forEach((row, y) => {
      ...
      row.forEach((cell, x) => {
        let resAlpha = (cell & 0xff) - this.value;
        if (resAlpha < 0) {
          resAlpha = 0;
        }
