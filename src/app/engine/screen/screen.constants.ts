export const SCREEN_WIDTH = 256;
export const SCREEN_HEIGHT = 192;
export const CELL_SIZE = 8;

export const LAYER_COUNT = 5;
export const BACKGROUND_LAYER = 0;
export const MIDDLE_TILE_LAYER = 1;
export const FOREGROUND_LAYER = 2;
export const UPPER_EFFECT_LAYER = 3;
export const HUD_LAYER = 4;

export const __ = 0x00000000;
export const Bk = 0x000000ff;
export const Wt = 0xffffffff;
export const Rd = 0xff0000ff;
export const Rk = 0x990000ff;
export const Gr = 0x00ff00ff;
export const Bl = 0x0000ffff;
export const Lg = 0xeeeeeeff;
export const Mg = 0xccccccff;
export const Dg = 0x888888ff;
export const Sg = 0x333333ff;
export const Or = 0xf8ab4aff;
export const Ol = 0xf6c77bff;
export const Yl = 0xffd700ff;
export const Yk = 0xff8845ff;
